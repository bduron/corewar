...
unsigned activateCDKUSlider (CDKUSLIDER *widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = (unsigned)injectCDKUSlider (widget, input);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
	 ret = (unsigned)injectCDKUSlider (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
#define SCANF_FMT "%u%c"
   ...
   if (temp != 0)
   {
	 ...
	 modify = FALSE;
	  ...
	  && value <= widget->high)
      {
	 setCDKUSliderValue (widget, (unsigned)value);
	 result = TRUE;
      }
      free (data);
   }
...
static int _injectCDKUSlider (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf (widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf (widget) (vUSLIDER,
					    widget,
   ...
   if (ppReturn != 0)
   {
	 ...
	 default:
      ...
      {
	 PostProcessFuncOf (widget) (vUSLIDER,
				     widget,
 ...
 */
static void _moveCDKUSlider (CDKOBJS *object,
			     int xplace,
			     int yplace,
			     boolean relative,
 ...
 */
void setCDKUSlider (CDKUSLIDER *widget,
		    unsigned low,
		    unsigned high,
		    unsigned value,
		    boolean Box)
