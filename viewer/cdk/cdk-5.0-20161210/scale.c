 ...
 */
static void drawCDKScaleField (CDKSCALE * widget);
			   ...
			   boolean shadow)
{
   ...
   if ((widget = newCDKObject (CDKSCALE, &my_funcs)) == 0)
      return (0);

   setCDKScaleBox (widget, Box);
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKObject (vSCALE,
		     widget,
		     (chtype) bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKObject (cdkscreen, vSCALE, widget);
 ...
 */
int activateCDKScale (CDKSCALE * widget, chtype *actions)
{
   int ret;
   ...
   if (actions == 0)
   {
	 ...
	 ret = (int) injectCDKScale (widget, input);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
	 ret = (int) injectCDKScale (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
 ...
 */
static void limitCurrentValue (CDKSCALE * widget)
{
 ...
 */
static int moveToEditPosition (CDKSCALE * widget, int newPosition)
{
 ...
 */
static int validEditPosition (CDKSCALE * widget, int newPosition)
{
   chtype ch;
 ...
 */
static void setEditPosition (CDKSCALE * widget, int newPosition)
{
   if (newPosition < 0)
   {
      Beep ();
 ...
 */
static bool performEdit (CDKSCALE * widget, chtype input)
{
   bool result = FALSE;
   bool modify = TRUE;
...
#define SCANF_FMT "%d%c"
   ...
   if (temp != 0)
   {
	 ...
	 modify = FALSE;
	  ...
	  && value <= widget->high)
      {
	 setCDKScaleValue (widget, (int) value);
	 result = TRUE;
      }
      free (temp);
   }
...
static int _injectCDKScale (CDKOBJS *object, chtype input)
{
   ...
   /* Draw the field. */
   drawCDKScaleField (widget);

   /* Check if there is a pre-process function to be called. */
   if (PreProcessFuncOf (widget) != 0)
   {
      /* Call the pre-process function. */
      ppReturn = PreProcessFuncOf (widget) (vSCALE,
					    widget,
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKObjectBind (vSCALE, widget, input) != 0)
      {
	 ...
	 default:
	    ...
	    else
	    {
	       ...
	       case 'd':
	       case '-':
		  return _injectCDKScale (object, KEY_DOWN);
	       case '+':
		  return _injectCDKScale (object, KEY_UP);
	       case 'D':
		  return _injectCDKScale (object, KEY_NPAGE);
	       case '0':
		  return _injectCDKScale (object, KEY_HOME);
	       default:
		  Beep ();
		  break;
	       }
      ...
      {
	 PostProcessFuncOf (widget) (vSCALE,
				     widget,
 ...
 */
static void _setBKattrScale (CDKOBJS *object, chtype attrib)
{
   if (object != 0)
   {
      CDKSCALE *widget = (CDKSCALE *) object;
...
static void _destroyCDKScale (CDKOBJS *object)
{
   if (object != 0)
   {
      ...
      cleanCDKObjectBindings (vSCALE, widget);

      /* Unregister this object. */
      unregisterCDKObject (vSCALE, widget);
   }
}
 ...
 */
void setCDKScale (CDKSCALE * widget, int low, int high, int value, boolean Box)
{
   setCDKScaleLowHigh (widget, low, high);
   setCDKScaleValue (widget, value);
   setCDKScaleBox (widget, Box);
}
 ...
 */
void setCDKScaleValue (CDKSCALE * widget, int value)
{
   widget->current = value;
   limitCurrentValue (widget);
}
int getCDKScaleValue (CDKSCALE * widget)
{
 ...
 */
void setCDKScaleLowHigh (CDKSCALE * widget, int low, int high)
{
...
int getCDKScaleLowValue (CDKSCALE * widget)
{
   return widget->low;
}
int getCDKScaleHighValue (CDKSCALE * widget)
{
   return widget->high;
}
 ...
 */
void setCDKScaleBox (CDKSCALE * widget, boolean Box)
{
   ObjOf (widget)->box = Box;
   ObjOf (widget)->borderSize = Box ? 1 : 0;
}
boolean getCDKScaleBox (CDKSCALE * widget)
{
   return ObjOf (widget)->box;
}

static void _focusCDKScale (CDKOBJS *object)
{
   CDKSCALE *widget = (CDKSCALE *) object;
...
static void _unfocusCDKScale (CDKOBJS *object)
{
   CDKSCALE *widget = (CDKSCALE *) object;
