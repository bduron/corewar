...
unsigned activateCDKUScale (CDKUSCALE * widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = (unsigned) injectCDKUScale (widget, input);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
	 ret = (unsigned) injectCDKUScale (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
#define SCANF_FMT "%u%c"
   ...
   if (temp != 0)
   {
	 ...
	 modify = FALSE;
	  ...
	  && value <= widget->high)
      {
	 setCDKUScaleValue (widget, (unsigned) value);
	 result = TRUE;
      }
      free (temp);
   }
...
static int _injectCDKUScale (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf (widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf (widget) (vUSCALE,
					    widget,
   ...
   if (ppReturn != 0)
   {
	 ...
	 default:
      ...
      {
	 PostProcessFuncOf (widget) (vUSCALE,
				     widget,
 ...
 */
static void _moveCDKUScale (CDKOBJS *object,
			     int xplace,
			     int yplace,
			     boolean relative,
