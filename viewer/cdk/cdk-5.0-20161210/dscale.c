...
double activateCDKDScale (CDKDSCALE * widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = (double) injectCDKDScale (widget, input);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
	 ret = (double) injectCDKDScale (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
#define SCANF_FMT "%lg%c"
   ...
   if (temp != 0)
   {
	 ...
	 modify = FALSE;
	  ...
	  && value <= widget->high)
      {
	 setCDKDScaleValue (widget, (double) value);
	 result = TRUE;
      }
      free (temp);
   }
...
static int _injectCDKDScale (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf (widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf (widget) (vDSCALE,
					    widget,
   ...
   if (ppReturn != 0)
   {
	 ...
	 default:
      ...
      {
	 PostProcessFuncOf (widget) (vDSCALE,
				     widget,
 ...
 */
static void _moveCDKDScale (CDKOBJS *object,
			     int xplace,
			     int yplace,
			     boolean relative,
