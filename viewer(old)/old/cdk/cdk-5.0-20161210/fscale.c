...
float activateCDKFScale (CDKFSCALE * widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = (float) injectCDKFScale (widget, input);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
	 ret = (float) injectCDKFScale (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
#define SCANF_FMT "%lg%c"
   ...
   if (temp != 0)
   {
	 ...
	 modify = FALSE;
	  ...
	  && value <= widget->high)
      {
	 setCDKFScaleValue (widget, (float) value);
	 result = TRUE;
      }
      free (temp);
   }
...
static int _injectCDKFScale (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf (widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf (widget) (vFSCALE,
					    widget,
   ...
   if (ppReturn != 0)
   {
	 ...
	 default:
      ...
      {
	 PostProcessFuncOf (widget) (vFSCALE,
				     widget,
 ...
 */
static void _moveCDKFScale (CDKOBJS *object,
			     int xplace,
			     int yplace,
			     boolean relative,
